...
import { Container, Row, Col, Button, FormGroup, Input, InputGroup, InputGroupButton, Table } from 'reactstrap';
...
class ManagerFullList extends Component {
  ...
  render() {
    ...
    let searchedItems = this.state.search? filteredItems.filter((item)=>{
    ...
    let fullList = sortedItems.map((item)=><ManagerItem key={item.id} item={item}/>)

    return (
      <div>
        <nav className='border secondary-nav'>
          <ul >
            <Link to={'/manageitems/forsale'}><li >For Sale</li></Link>
            <Link to={'/manageitems/onhold'}><li>On Hold</li></Link>
            <Link to={'/manageitems/sold'}><li>Sold</li></Link>
          </ul>
        </nav>
        <div className='itemSearch'>
          <Container className='pl-5'>
            <Row>
              ...
              <Col>
                <Link to={'/manageradditem'}><Button>Add Item</Button></Link>
              </Col>
            </Row>
          </Container>
        </div>
