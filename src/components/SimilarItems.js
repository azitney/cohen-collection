...
import { Container, Card, CardImg, CardTitle,
...
class SimilarItems extends Component {
  ...
  render() {
    let filtereditems = this.props.items.filter((item)=> item.id != this.props.itemid)

    let itemone = filtereditems[Math.floor((Math.random() * filtereditems.length))]

    let filteredtwoitems = filtereditems.filter((item)=> item.id != itemone.id)

    let itemtwo = filteredtwoitems[Math.floor((Math.random() * filteredtwoitems.length))]

    let filteredthreeitems = filteredtwoitems.filter((item)=> item.id != itemtwo.id)

    let itemthree = filteredthreeitems[Math.floor((Math.random() * filteredthreeitems.length))]
